...
import (
	...
	model "github.com/MahadevMaahi/gin-jwt-authentication/models"
...
func VerifyPassword(userPassword string, providedPassword string) (bool, string) {
	...
	if err != nil {
		msg = fmt.Sprintf("the password is incorrect")
		check = false
	}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error" : "email is not registered"})
			return
		}

		if foundUser.Email == nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error" : "email is not found"})
			return
		}
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context){
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error" : "error occured while retriving user items"})
			return
		}
